...
def formatting(sheet, col_val):
    ...
    workbook_writer = writer.book
    worksheet_writer = writer.sheets[sheet]

    # Add accountancy format
    accounting = workbook_writer.add_format({'num_format': '_($* #,##0.00_);_($* (#,##0.00);_($* ""-""??_);_(@_)'})

    # Set format without assigning column width
    worksheet_writer.set_column(col_val + ':' + col_val, None, accounting)
...
# Define a function to change PaymentRefNumber and save each piece
def payrefnum_change(current_number):
    pound_loc = current_number.find('#')
    check_reference_number = current_number[pound_loc - 2:]

    return check_reference_number
...
for typ in Type:
    ...
    if typ == 'Check':
        ...
        check_det = List[List['Type'].str.contains('Check Detail', regex=False)]
        ...
        if not check_op.empty:
            ...
            for i in range(len(check_op['Type'])):
                ...
                # Expense Account
                # Sometimes there are names that are backwards. This may be the case if the name is of an employee.
                # Catch this case, and determine where the match is.
                exp_acc_det = check_det[check_det['To/From'] == check_op['To/From'].iloc[i]]

                if not exp_acc_det.empty:
                    exp_acc = bank_account_name(exp_acc_det)
                # If there are multiple checks with the same name, find the corresponding amount for the Check Detail.
                elif len(exp_acc_det['Type']) > 1:
                    exp_acc_det = exp_acc_det[exp_acc_det['Debit'] == check_op['Credit'].iloc[i]]
                    exp_acc = bank_account_name(exp_acc_det)
                # If there are no matches, check for backwards names, or save for later analysis.
                else:
                    name_poss = check_op['To/From'].iloc[i].split()
                    name_flip = ' '.join(reversed(name_poss))
                    exp_acc_det = check_det[check_det['To/From'] == name_flip]
                    exp_acc = bank_account_name(exp_acc_det)
                    if exp_acc_det.empty:
                        # There is no match. Print this out for later evaluation.
                        exp_diff_new = check_op.iloc[i]
                        exp_diff = exp_diff.append(exp_diff_new, ignore_index=True)
    ...
    if typ == 'Deposit':
        ...
        if not payments.empty:
            ...
            for i in range(len(payments['Type'])):
                ...
                # Change PaymentRefNumber if the name is not in the form ck# 358 or mo# 358
                if isnan(payments['No'].iloc[i]):
                    new_pay_ref = payments['No'].iloc[i]
                    note = payments['Description'].iloc[i]
                elif payments['No'].iloc[i].find('#') == 2:
                    new_pay_ref = payments['No'].iloc[i]
                    note = payments['Description'].iloc[i]
                else:
                    new_pay_ref = payrefnum_change(payments['No'].iloc[i])
                    note = payments['Description'].iloc[i] + ' ' + payments['No'].iloc[i]

                Payments_new = pd.Series(
                    [ref_pay, payments['Date'].iloc[i], new_pay_ref, payments['To/From'].iloc[i],
                     acc_payments[0], payments['Debit'].iloc[i], note, '', '', '', ''])
    ...
    if typ == 'Journal':
        if not List.empty:
            # Take note of No column for proper separation.
            private_note = List['No'].unique()
            for priv in private_note:
                journ_private = List[List['No'].str.contains(priv, regex=False)]

                # Sort debits in descending order and credits in ascending order.
                debit_credit = ['Debit', 'Credit']
                journ_priv = pd.DataFrame()
                for dc in debit_credit:
                    journ_priv_new = journ_private[pd.notna(journ_private[dc])]
                    if dc == 'Debit':
                        order = False
                    else:
                        order = True
                    journ_priv_new = journ_priv_new.sort_values(by=dc, ascending=order)
                    journ_priv = journ_priv.append(journ_priv_new, ignore_index=True)

                # Increment ref number
                ref_journal += 1

                for i in range(len(journ_priv['Type'])):

                    # If Debit, amount is positive. If Credit, amount is negative.
                    if pd.notna(journ_priv['Credit'].iloc[i]):
                        # Credit
                        amount = -journ_priv['Credit'].iloc[i]
                        ...
                        # Debit
                        amount = journ_priv['Debit'].iloc[i]

                        # Change Deposit Account name
                        acc_journal = bank_account_name(journ_priv.iloc[[i]])

                    Journals_new = pd.Series(
                        [ref_journal, journ_priv['Date'].iloc[i], journ_priv['No'].iloc[i], acc_journal[0], amount,
                         journ_priv['Description'].iloc[i], 'Pawley\'s Island', '', '', '', '', ''])
...
for i in sheet_names:
    ...
    if not exp_diff.empty:
        exp_diff.to_excel(writer, index=False, sheet_name='Missing Check')

        # Get the xlsxwriter workbook and worksheet objects.
        workbook = writer.book
        worksheet = writer.sheets['Missing Check']

        # Note: It isn't possible to format any cells that already have a format such
        # as the index or headers or any cells that contain dates or datetimes.

        # Auto-adjust columns' width
        get_col_widths(exp_diff, 'Missing Check')
